...
import type { HourlyRisk, RiskLevel } from "./types";
...
export function RiskPredictionChart({ hourly, maxBarHeight = 160 }: RiskPredictionChartProps) {
  ...
  return (
    <div className="relative flex flex-col gap-(--gap-3) rounded-2xl bg-bg-surface px-(--padding-3) py-(--padding-8)">
      <span className="absolute top-[16px] left-[16px] text-caption-small-pc">
          ...
          {hourly.map((item) => {
            ...
            return (
              <div className="relative" key={item.timeFrame}>
                ...
                />
                <div
                  className="absolute left-1/2 flex -translate-x-1/2 items-start gap-[4px] text-body-medium-pc whitespace-nowrap"
      ...
      <div className="grid auto-cols-fr grid-flow-col">
        {hourly.map((item) => (
          <div className="flex items-center justify-center gap-(--gap-2)" key={item.timeFrame}>
            <span className="text-caption-medium-pc text-text-secondary">{item.label}</span>
            <Badge status={item.risk}>{RISK_LABEL[item.risk]}</Badge>
          </div>
        ))}
      </div>
    </div>
